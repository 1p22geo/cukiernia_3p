...
export const ProductInfo = ({ product }: { product: Product }) => {
  return <div>
    <a href="/" className="text-right w-full block hover:underline">Wróć do produktów</a>
    ...
    <div className="flex flex-row w-full overflow-x-scroll">
      {
        Array.from(Array(10).keys()).map(k =>
          <img src={`https://picsum.photos/400?${k}`} width={200} key={k} className="rounded-xl p-2" />)
      }
    </div>
